...
export OH_AUTO_PROFILE_ID=musepaper2
export OH_AUTO_FLASH_TEMPLATE_ID=musepaper2-titan
export OH_AUTO_HDC_TARGET=0123456789ABCDEF
export OH_AUTO_WIFI_DIAG_PATH=/system/bin/muse_wifi_diag
```
...
- MusePaper2 OH6.1 images from iteration255 onward include the WiFi diagnostic
  helper at `/system/bin/muse_wifi_diag`. Set `OH_AUTO_WIFI_DIAG_PATH` to this
  path or pass `--diag-path /system/bin/muse_wifi_diag`. For older images where
  the helper was pushed into `/data/local/tmp`, pass
  `--diag-path /data/local/tmp/muse_wifi_diag`.
