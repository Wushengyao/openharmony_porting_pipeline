...
def build_install_command(args: argparse.Namespace, suite_dir: str) -> str:
    ...
    script = (
        ...
        "Sort-Object Name | Group-Object { "
        "$_.Name.ToLowerInvariant().Replace('_','-').Replace('.tar.gz','') "
        "} | ForEach-Object { "
        "$_.Group | Sort-Object @{Expression={ if ($_.Name -like '*_*') { 1 } else { 0 } }},Name | "
        "Select-Object -First 1 "
        "} | Sort-Object Name | ForEach-Object { $_.FullName }; "
        "if (-not $packages -or $packages.Count -eq 0) { "
        "throw 'no xdevice*.tar.gz packages found under suite tools directory' "
        "}; "
        "Write-Host ('installing xDevice packages: ' + ($packages -join ', ')); "
